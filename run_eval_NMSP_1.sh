...
nohup env CUDA_VISIBLE_DEVICES=3 python fake_quant/main.py \
    ...
    --act_sparsity 2:4 \
    --act_sparsity_location pre_rotate \
    --weight_scoring \
    --target_modules "${config}" \
    --no-sparsity_calibration \
    >> "$LOG_DIR/eval_w4a4_NMSP_1_nocal.log" 2>&1 &
...
nohup env CUDA_VISIBLE_DEVICES=3 python fake_quant/main.py \
    ...
    --target_modules "${config}" \
    --no-sparsity_calibration \
    >> "$LOG_DIR/eval_w8a8_NMSP_1_nocal.log" 2>&1 &
...
nohup env CUDA_VISIBLE_DEVICES=0 python fake_quant/main.py \
    ...
    --target_modules "${config}" \
    --no-sparsity_calibration \
    >> "$LOG_DIR/eval_bf16_NMSP_1_nocal.log" 2>&1 &
