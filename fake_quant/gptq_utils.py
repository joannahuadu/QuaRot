...
@torch.no_grad()
def gptq_fwrd__wo_replcaing_weight(model, dataloader, dev, args):
    '''
    GPTQ forward pass that returns compressed weights without replacing model weights.
    '''
    logging.info('-----GPTQ Quantization (no weight replace)-----')
    ...
    utils.cleanup_memory(verbos=True)
    logging.info('-----GPTQ Quantization Done-----\n')
    return quantizers, compressed_weights
